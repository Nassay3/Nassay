...
def main():
    ...
    for cfg in cfgs:
        sg=c.signal(x,cfg); cache[cfg]=sg; t=c.simulate(x,sg,ex0); tr,va,oo=c.three(t)
    ...
    for _,r in good.iterrows():
        cfg=c.SigCfg(15,str(r.setup),str(r.trend),str(r.vwap),float(r.zthr),str(r.zslow)); sg=cache[cfg]
        for am in (1.5,2.0,2.5,3.0):
            for man in ('2R','3R','4R','FIB','EXT'):
                ex=c.ExitCfg(am,man); t=c.simulate(x,sg,ex); tr,va,oo=c.three(t); key=f'{cfg.setup}-{cfg.trend}-{cfg.vwap}-z{cfg.zthr}-{cfg.zslow}-a{am}-{man}'; trades[key]=t
