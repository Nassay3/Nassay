...
def stop_for(x,i,d,entry):
    ...
    if d==1:
        swing=min(float(x.low.iloc[i]),float(x.swinglo6.iloc[i])); st=min(swing-.1*av,entry-.85*av); r=entry-st
    else:
        swing=max(float(x.high.iloc[i]),float(x.swinghi6.iloc[i])); st=max(swing+.1*av,entry+.85*av); r=st-entry
...
def simulate(x,L,S,start,end,beR,trail,capR,max_hold,max_lots=4):
    ...
    for i in range(max(first,30),min(last,len(x)-2)+1):
        ...
        for z in lots:
            # Trailing value is based only on information available before this bar.
            active=z.stop
            if z.hit4:
                if trail=='R2': active=max(active,z.entry,z.best-2*z.risk) if z.d==1 else min(active,z.entry,z.best+2*z.risk)
                elif trail=='VWMA84':
                    tv=float(x.vwma84.iloc[i-1]);active=max(active,z.entry,tv) if z.d==1 else min(active,z.entry,tv)
            ...
            if (not z.hit2) and fav>=2:
                part=.2;gain=part*2;bal+=z.rc*gain;z.realized+=gain;z.rem-=part;z.hit2=True
            if (not z.hit4) and fav>=4:
                part=.2;gain=part*4;bal+=z.rc*gain;z.realized+=gain;z.rem-=part;z.hit4=True
            ...
            z.best=max(z.best,hi) if z.d==1 else min(z.best,lo)
            surv.append(z)
        lots=surv
        if not lots:cdir=0
        eq=bal
...
def main():
    x=g.prep();L,S=signal(x);rows=[]
    for be in [1.,2.]:
      for trl in ['R2','VWMA84','M15VWMA21','M15VWMA175']:
       for cap in [12.,16.]:
        for mh in [144,288]:
            tr=simulate(x,L,S,g.START,g.TRAIN_END,be,trl,cap,mh);va=simulate(x,L,S,g.TRAIN_END,g.VAL_END,be,trl,cap,mh)
            if not tr or not va:continue
            score=tr['ret']-1.1*tr['dd']+.7*va['ret']-.8*va['dd'];rows.append((score,be,trl,cap,mh,tr,va))
    rows.sort(reverse=True,key=lambda z:z[0]);out=[]
    for score,be,trl,cap,mh,tr,va in rows:
        if va['ret']>0 and va['pf']>=1.08:
            ho=simulate(x,L,S,g.VAL_END,g.END,be,trl,cap,mh);full=simulate(x,L,S,g.START,g.END,be,trl,cap,mh)
            out.append({'beR':be,'trail':trl,'capR':cap,'max_hold_bars':mh,'score':score,'train':tr,'val':va,'holdout':ho,'full':full})
    print('RESULT_JSON_START');print(json.dumps({'risk_pct':.36,'partials':'20%@2R + 20%@4R + 60% runner','selection':'train+validation only','results':out[:12],'target_1000_R':math.log(11)/g.RISK,'limitations':['M1 OHLC->M5','0.10R cost/lot for partial-exit structure','one unprotected lot max','trails use prior-bar values','no news/DXY']},default=float));print('RESULT_JSON_END')
